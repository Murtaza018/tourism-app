    ...
    try {
      ...
      const flightPackageQuery = `
        SELECT
          ...
          fp.end_date,
          fp.status,
          f1.email AS airline_email,
          f2.email AS return_airline_email
        FROM
          flight_package fp
        LEFT JOIN
      ...
      for (const packageRow of flightPackageResults) {
        ...
        const packageObj = {
          ...
          airline_email: packageRow.airline_email, // Added airline_email
          return_airline_email: packageRow.return_airline_email, // Added return_airline_email
        };
...
const insertPackageFlight = async (req, res) => {
  await FlightPackageCheckTable();
  ...
  pool.getConnection(async (err, connection) => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        connection.query(
          `update flight_package set return_flight_id=?,end_date=? where package_id=?`,
          [req.body.flight_id, req.body.return_date, req.body.package_id],
          (err, results) => {
            if (err) {
              reject(err);
            } else {
      ...
      await new Promise((resolve, reject) => {
        connection.query(
          `insert into flight_reservation(tourist_email,airline_email,status,seats_booked,flight_id,price) values(?,?,"pending",?,?,?)`,
          [
            req.body.email,
            req.body.flight_email,
            req.body.quantity,
            req.body.flight_id,
            req.body.price,
          ],
      ...
      await new Promise((resolve, reject) => {
        connection.query(
          `update flight set seats_available=seats_available-? where flight_id=?`,
          [req.body.quantity, req.body.flight_id],
          (err, results) => {
            if (err) {
              reject(err);
            } else {
...
const CheckPackageCount = async (req, res) => {
  await FlightPackageCheckTable();
  pool.query(
    `select count(package_id) as package_count from flight_package where tourist_email=? and (status="pending" or status="ongoing");`,
    [req.body.email],
    (err, results) => {
      if (results) {
